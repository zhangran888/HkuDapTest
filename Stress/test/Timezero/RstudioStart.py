    ...
    while True:
        try:
            ...
            element_open = driver.find_element(by=By.XPATH,
                                               value='//button[@class="el-button el-button--text el-button--large"]/span[text()=" Start "]')
            element_open.click()
            print("#############################【线程" + str(
                prid) + username + " - start元素打开成功】######################")

            break
        except Exception as e:
            print(
                "#############################【线程" + str(prid) + username + "--->start元素失败】######################")
            retry_count += 1
            if retry_count > MAX_RETRY_COUNT:
                print("【" + username + "--->start打开重试超时，超过重试次数】")
                return 500
            driver.refresh()
    while True:
        try:
            element_Running = driver.find_element(by=By.XPATH,
                                                  value='//span[contains(text(),"Running")]')
            if element_Running:
                print("#############################【线程" + str(
                    prid) + username + " - Running元素获取成功】######################")
                time.sleep(5)
                driver.close()
                break
        except Exception as e:
            print(
                "#############################【线程" + str(
                    prid) + username + "--->Running元素获取失败】######################")
            retry_count += 1
            if retry_count > MAX_RETRY_COUNT:
                print("【" + username + "--->Running获取超时，超过重试次数】")
                return 500
            driver.refresh()
